...
When a reply arrives after publication, follow the postpublication update workflow in [`docs/EDITORIAL_WORKFLOW.md`](docs/EDITORIAL_WORKFLOW.md#updating-a-published-investigation). Authenticate and preserve the complete exchange privately, distinguish a response update from a factual correction, reconcile the case file, revise every affected summary/body/Ledger field, obtain editor approval, and repeat the technical deployment and live-verification gates. Later outreach never retroactively becomes prepublication right of reply.
